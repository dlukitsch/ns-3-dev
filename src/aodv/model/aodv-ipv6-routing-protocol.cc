...
#include "aodv-ipv6-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("AodvIpv6RoutingProtocol");
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::aodvIpv6::DeferredRouteOutputTag")
      .SetParent<Tag> ()
      .SetGroupName ("AodvIpv6")
...
RoutingProtocol::RoutingProtocol ()
  : m_rreqRetries (2),
    ...
    m_netDiameter (35),
    m_nodeTraversalTime (MilliSeconds (60)),
...
{
  static TypeId tid = TypeId ("ns3::aodvIpv6::RoutingProtocol")
                          ...
                          LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  for (std::map<Ptr<Socket>, Ipv6InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      if (m_ipv6->GetInterfaceForAddress (iface.GetAddress ()) == iif)
        {
          if (dst == Ipv6Address::GetAllNodesMulticast() || dst.IsAllNodesMulticast ())
            {
                  ...
                  if (m_routingTable.LookupRoute (dst, toBroadcast))
                    {
                      ...
                      ucb (m_ipv6->GetNetDevice(iif), route, packet, header);
                    }
                  else
                    {
  ...
  // Forwarding
  return Forwarding (iif, p, header, ucb, ecb);
}

bool
RoutingProtocol::Forwarding (int32_t iif, Ptr<const Packet> p, const Ipv6Header & header,
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  NS_LOG_FUNCTION (this);
  Ipv6Address dst = header.GetDestinationAddress ();
      ...
      if (toDst.GetFlag () == VALID)
        {
          ...
          ucb (m_ipv6->GetNetDevice(iif),route, p, header);
          return true;
        }
      else
        {
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvAodv, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (Inet6SocketAddress (Ipv6Address::GetAllNodesMulticast(), AODV_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
  ...
  RoutingTableEntry rt (/*device=*/ dev, /*dst=*/ Ipv6Address::GetAllNodesMulticast(), /*know seqno=*/ true, /*seqno=*/ 0, /*iface=*/ iface,
                                    /*hops=*/ 1, /*next hop=*/ Ipv6Address::GetAllNodesMulticast(), /*lifetime=*/ Simulator::GetMaximumSimulationTime ());
  m_routingTable.AddRoute (rt);

  if (l3->GetInterface (i)->GetNdiscCache ())
    {
      m_nb.AddArpCache (l3->GetInterface (i)->GetNdiscCache ());
    }
      ...
      if (mac != 0)
        {
          ...
          m_nb.DelArpCache (l3->GetInterface (i)->GetNdiscCache ());
        }
    }
      ...
      if (!socket)
        {
          ...
          RoutingTableEntry rt (/*device=*/ dev, /*dst=*/ Ipv6Address::GetAllNodesMulticast(), /*know seqno=*/ true,
                                            /*seqno=*/ 0, /*iface=*/ iface, /*hops=*/ 1,
                                            /*next hop=*/ Ipv6Address::GetAllNodesMulticast(), /*lifetime=*/ Simulator::GetMaximumSimulationTime ());
          m_routingTable.AddRoute (rt);
        }
    }
  else
  ...
  for (std::map<Ptr<Socket>, Ipv6InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      if (iface.GetPrefix () == Ipv6Prefix::GetOnes ())
        {
          destination = Ipv6Address::GetOnes();
        }
      else
        {
...
{
  socket->SendTo (packet, 0, Inet6SocketAddress (destination, AODV_PORT));
...
RoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv6Address receiver, Ipv6Address src)
{
   ...
   *  6. the hop count is copied from the Hop Count in the RREQ message;
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
       ...
       */
      if ((int32_t (toDst.GetSeqNo () - int32_t (rreqHeader.GetDstSeqno ()) >= 0))
          && toDst.GetValidSeqNo () )
        {
          if (toDst.GetFlag () == VALID)
            {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
   ...
   */
  if ((rreqHeader.GetDstSeqno () == m_seqNo + 1))
    {
      m_seqNo++;
    }
  ...
  while (m_queue.Dequeue (dst, queueEntry))
    {
      ...
      ucb (m_ipv6->GetNetDevice(m_ipv6->GetInterfaceForDevice (route->GetOutputDevice ())), route, p, header);
    }
}
  ...
  for (std::vector<Ipv6InterfaceAddress>::const_iterator i = ifaces.begin (); i != ifaces.end (); ++i)
    {
      ...
      Ipv6Address destination;
      if (i->GetPrefix () == Ipv6Prefix::GetOnes ())
      {
        destination = Ipv6Address::GetOnes();
      }
      else
